export const EMAIL_SENDERS = {
  WELCOME: "Welcome <welcome@service.pair-form.com>",
  NOTIFY: "PairForm <notify@service.pair-form.com>",
  SUPPORT: "PairForm Support <support@service.pair-form.com>",
};

export const getWelcomeEmail = (name: string) => {
  return {
    from: EMAIL_SENDERS.WELCOME,
    subject: "Welcome to PairForm!",
    html: `
      <div style="font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; color: #333; max-width: 600px; margin: 0 auto; line-height: 1.6;">
        ...
        <h1 style="color: #111; font-size: 24px; font-weight: 700;">Hi there, ${name} </h1>
        
        <p style="font-size: 16px;">Welcome to <strong>PairForm</strong>. We are excited to have you on board!</p>
        
        <p style="font-size: 16px;">PairForm helps you create fair, balanced groups without the usual back-and-forth, spreadsheets, or confusion. In just a few minutes, you can set up slots, invite participants, and let everyone place themselves where they fit best.</p>
        
        <h3 style="font-size: 18px; font-weight: 600; margin-top: 30px;">Here is what you can do next:</h3>
        
        <ul style="list-style: none; padding: 0;">
          <li style="margin-bottom: 12px; display: flex; align-items: flex-start;">
            <span style="color: #3b82f6; font-size: 20px; margin-right: 12px; line-height: 1;">&bull;</span>
            <span style="padding-top: 2px;"><strong>Create your first event</strong> or grouping</span>
          </li>
          <li style="margin-bottom: 12px; display: flex; align-items: flex-start;">
            <span style="color: #3b82f6; font-size: 20px; margin-right: 12px; line-height: 1;">&bull;</span>
            <span style="padding-top: 2px;"><strong>Invite participants</strong> with a simple link</span>
          </li>
          <li style="margin-bottom: 12px; display: flex; align-items: flex-start;">
            <span style="color: #3b82f6; font-size: 20px; margin-right: 12px; line-height: 1;">&bull;</span>
            <span style="padding-top: 2px;"><strong>Set roles, limits, and rules</strong> to keep things balanced</span>
          </li>
          <li style="margin-bottom: 12px; display: flex; align-items: flex-start;">
            <span style="color: #3b82f6; font-size: 20px; margin-right: 12px; line-height: 1;">&bull;</span>
            <span style="padding-top: 2px;"><strong>Go live</strong> and let PairForm handle the rest</span>
          </li>
        </ul>
        
        <p style="font-size: 16px; margin-top: 30px;">If you ever get stuck, need ideas, or want to explore advanced use cases (team formation, Secret Santa, one on one pairing, number/postion assigning workshops, games, and more), we’re here to help.</p>
        
        <p style="font-size: 16px;">Thanks for choosing PairForm, we can not wait to see what you create.</p>
        ...
        <p style="font-size: 14px; color: #666;">
          ...
          <span style="font-style: italic; color: #888;">Helping people form better groups, faster</span>
        </p>
      </div>
    `,
  };
...
export const getPairingEmail = (
  ...
  const typeText = isSecretSanta ? "Secret Santa Match" : "Pairing Result";
  const actionText = isSecretSanta ? "gifting to" : "paired with";

  return {
    from: EMAIL_SENDERS.NOTIFY,
    subject: `${typeText}: ${eventName}`,
    html: `
      <div style="font-family: sans-serif; color: #333;">
        <h1>${eventName} Results</h1>
        <p>Hi ${participantName},</p>
        <p>The pairings have been generated!</p>
        <div style="padding: 20px; background-color: #f5f5f5; border-radius: 5px; margin: 20px 0;">
          <p style="font-size: 18px; margin: 0;">You are ${actionText}: <strong>${matchName}</strong></p>
          ${
            matchDetails
              ? `<p style="margin-top: 10px; color: #666;">Details: ${matchDetails}</p>`
              : ""
          }
        </div>
        <p><a href="https://pair-form.com">View your pairing online</a></p>
        <br/>
        <p>Best,<br/>The Pair Form Team</p>
      </div>
    `,
  };
};
    ...
    html: `
      <div style="font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; color: #333; max-width: 600px; margin: 0 auto; line-height: 1.6;">
        ...
        <ul style="list-style: none; padding: 0;">
          <li style="margin-bottom: 12px; display: flex; align-items: flex-start;">
            <span style="color: #3b82f6; font-size: 20px; margin-right: 12px; line-height: 1;">&bull;</span>
            <span style="padding-top: 2px;">The organizer will complete participant collection</span>
          </li>
          <li style="margin-bottom: 12px; display: flex; align-items: flex-start;">
            <span style="color: #3b82f6; font-size: 20px; margin-right: 12px; line-height: 1;">&bull;</span>
            <span style="padding-top: 2px;">Groups will be finalized automatically</span>
          </li>
          <li style="margin-bottom: 12px; display: flex; align-items: flex-start;">
            <span style="color: #3b82f6; font-size: 20px; margin-right: 12px; line-height: 1;">&bull;</span>
            <span style="padding-top: 2px;">You’ll receive an update or shared link once everything is ready</span>
          </li>
        </ul>

        <p style="font-size: 16px; margin-top: 30px;">If you need to update your details or have questions, please reach out to the organizer directly.</p>
        
        <p style="font-size: 16px;">Thanks for taking part — we’re excited to have you involved!</p>
        ...
        <p style="font-size: 14px; color: #666;">
          Best,<br/>
          <strong>The PairForm Team</strong><br/>
          <span style="font-style: italic; color: #888;">Simple. Fair. Stress-free grouping.</span>
        </p>
      </div>
    `,
  };
...
export const getGroupCompleteEmail = (
  shareLink: string,
  groupName: string,
  participants: any[]
) => {
  const membersList = participants
    .map(
      (p) =>
        `<li style="margin-bottom: 8px; color: #333;"><strong>${
          p.name
        }</strong>${
          p.role || p.track
            ? `  <span style="color: #666;">(${p.role || p.track})</span>`
            : ""
        }</li>`
    )
    .join("");
  ...
  return {
    from: EMAIL_SENDERS.NOTIFY,
    subject: `Your group is complete: ${groupName}`,
    html: `
      <div style="font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; color: #333; max-width: 600px; margin: 0 auto; line-height: 1.6;">
        <div style="text-align: center; margin-bottom: 30px;">
 <svg
      ...
      >
        ...
        <h1 style="color: #111; font-size: 24px; font-weight: 700;">Great news 👏</h1>
        
        <p style="font-size: 16px;">All participants have successfully joined <strong>${groupName}</strong> your group is now complete!</p>
        
        <div style="background-color: #f9fafb; padding: 20px; border-radius: 8px; margin: 20px 0; border: 1px solid #e5e7eb;">
          <h3 style="margin-top: 0; font-size: 18px; color: #111;">Group Members:</h3>
          <ul style="padding-left: 20px; margin-bottom: 0;">
            ${membersList}
          </ul>
        </div>
        
        <p style="font-size: 16px;">Everything is locked in and ready to go. You can view the full dashboard using the link below:</p>
        
        <div style="margin: 30px 0; text-align: center;">
          <a href="${shareLink}" style="background-color: #3b82f6; color: white; padding: 12px 24px; text-decoration: none; border-radius: 6px; font-weight: bold; font-size: 16px;">🔗 View & share your group</a>
        </div>
        
        <p style="font-size: 16px;">Best of luck — and have a great session!</p>
